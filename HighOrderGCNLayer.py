...
class GraphConvolution(nn.Module):
    def __init__(self, in_features, out_features, edge_types, dropout=0.5, bias=True, use_bn=False,
        ...
        self.GraphConv2 = nn.ModuleList()
        self.GraphConv3 = nn.ModuleList()
        # batch norm
        self.use_bn = use_bn
        if self.use_bn:
        ...
        for _ in range(edge_types):
            ...
            self.GraphConv.append(BottledOrthogonalLinear(in_features=in_features,
                                                          ...
                                                          bias=bias))
            self.GraphConv2.append(BottledOrthogonalLinear(in_features=in_features,
                                                          out_features=out_features,
                                                          bias=bias))
            self.GraphConv3.append(BottledOrthogonalLinear(in_features=in_features,
                                                          out_features=out_features,
                                                          bias=bias))
        self.device = device
        self.to(device)

    def forward(self, input, adj, cat=False):
        """
        ...
        adjj = adj_.float()
        adj_2 = torch.matmul(adjj, adjj).to(self.device)
        adj_3 = torch.matmul(adj_2, adjj).to(self.device)
        ts, ts2, ts3  = [], [], []
        for i in range(self.edge_types):
            ...
            #二阶依存
            adj_hat_i2 = adj_2[i] * gate_status  # (batch_size, seq_len, seq_len)
            ts2.append(torch.bmm(adj_hat_i2.float(), self.GraphConv2[i](input)))

            #三阶依存
            adj_hat_i3 = adj_3[i] * gate_status  # (batch_size, seq_len, seq_len)
            ts3.append(torch.bmm(adj_hat_i3.float(), self.GraphConv3[i](input)))
        # 三种边的信息聚合
        if cat:
            ts = torch.stack(ts).sum(dim=0, keepdim=False).to(self.device)
            ts2 = torch.stack(ts2).sum(dim=0, keepdim=False).to(self.device)
            ts3 = torch.stack(ts3).sum(dim=0, keepdim=False).to(self.device)
            ts = torch.cat((ts,ts2,ts3),dim=-1)
        else:
            ts = ts + ts2 +ts3
            ts = torch.stack(ts).sum(dim=0, keepdim=False).to(self.device)
